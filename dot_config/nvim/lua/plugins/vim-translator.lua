...
return {
  {
    ...
    keys = {
      { "v", "<Leader>t",  desc = "Translate selection (window)" },
      { "v", "<Leader>T",  desc = "Translate selection (echo)" },
      { "n", "<Leader>tw", desc = "Translate word (window)" },
      { "n", "<Leader>tW", desc = "Translate word (echo)" },
    },
    config = function()
      ...
      local function get_selection()
        ...
        if mode == "V" then start_col, end_col = 1, #vim.fn.getline(end_row) end
      ...
      local function translate(text, display_mode)
        ...
        if cyrillic:match_str(text) then source_lang, target_lang = "ru", "en" end
        if display_mode == "window" then configure_window() end
      ...
      vim.keymap.set("v", "<Leader>t", function() translate(get_selection(), "window") end, { desc = "Translate selection (window)" })
      vim.keymap.set("v", "<Leader>T", function() translate(get_selection(), "echo") end, { desc = "Translate selection (echo)" })
      vim.keymap.set("n", "<Leader>tw", function() translate(vim.fn.expand "<cword>", "window") end, { desc = "Translate word (window)" })
      vim.keymap.set("n", "<Leader>tW", function() translate(vim.fn.expand "<cword>", "echo") end, { desc = "Translate word (echo)" })
    end,
  },
}
