...
-- the choice is persisted and restored on startup
...
local function theme_picker()
  ...
  vim.ui.select(list, { prompt = 'Colorscheme' }, function(choice)
    if choice and require('config.functions').apply_colorscheme(choice) then
      require('config.functions').save_colorscheme(choice)
    end
  end)
end
