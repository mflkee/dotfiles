...
local function enabled()
  ...
  if vim.g.dsync_capture_enabled == false or vim.g.dsync_capture_enabled == 0 then return false end
...
local function should_fire(path)
  if path:sub(1, #home) ~= home then return false end
  if vim.fn.filereadable(path) ~= 1 then return false end
  -- правки прямо в исходнике dotfiles
  if path:sub(1, #dotfiles_src) == dotfiles_src then return true end
...
local function fire(path)
  ...
  if last_fired[path] and now - last_fired[path] < DEBOUNCE_NS then return end
  last_fired[path] = now
...
vim.api.nvim_create_autocmd('BufWritePost', {
  ...
  callback = function(ev)
    ...
    if should_fire(path) then fire(path) end
  end,
})
