...
| `zen` | remote | Zen-браузер (`http://127.0.0.1:8877/mcp`): вкладки в контейнерах (`main`), JS-консоль, навигация. Источник: `~/projects/zen-mcp` (git-репо из dsync; расширение MV2 в профиле Zen + systemd user `zen-mcp.service`). Установка на машину: `git clone` + `bash ~/projects/zen-mcp/install.sh` + перезапуск Zen. Headless-машины: сервер работает, инструменты отвечают «extension not connected» |
